...
class MyNearestNeighborsRLAgent(MyTabularRLAgent):
    ...
    def update(self, observations, action, r, max_value):
        ...
        distances = self.distance(observations, neighbors)
    ...
    def weight (self, observations, distances):
       ...
       for item in distances:
           sum_all += item[1]
       for item in distances:
           weights.append((item[0], 1 - (item[1] / sum_all)))
       weights.sort(key=lambda tup : tup[1])
       print("weights: ", weights)
       return weights


    def distance (self, observations, neighbors):
       """
       ...
       for item in neighbors:
           fine_loc = get_environment().maze.rc2xy(item[0], item[1])
           distance = math.sqrt((fine_loc[0] - o[0]) ** 2 + (fine_loc[1] - o[1]) ** 2)
           neighbor_distance.append((item, distance))

       neighbor_distance.sort(key=lambda tup : tup[1]) 
       print("distances: ", neighbor_distance[:3])
    ...
    def neighbors (self, observations, action):
        ...
        o = tuple([x for x in observations])
        #convert x,y position to tile position
        coarse_loc = self.fine2coarse(observations, action) #get_environment().maze.xy2rc(o[0], o[1])
        r1 = coarse_loc[0]
        c1 = coarse_loc[1]
        neighbors = []
        for r in range(-1, 2):
